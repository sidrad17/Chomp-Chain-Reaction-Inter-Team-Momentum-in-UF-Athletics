...
def clean_wikipedia(df, sport_name, championship_years=None):
    ...
    if 'wins' in df.columns:
        df = df[~df['wins'].astype(str).str.contains(
            'No games|canceled|cancelled', case=False, na=False)].copy()
    print(f"  After filtering invalid: {before} -> {len(df)}")
...
if tennis_dfs:
    ...
    if not tennis_raw.empty:
        ...
        if not cleaned_tennis.empty:
            ...
            print(f'✓ Tennis: {len(cleaned_tennis)} seasons | {cleaned_tennis["year"].min()} → {cleaned_tennis["year"].max()}')
...
if wtennis_dfs:
    ...
    if not wtennis_raw.empty:
        ...
        if not cleaned_wtennis.empty:
            ...
            print(f'✓ Women\'s Tennis: {len(cleaned_wtennis)} seasons | {cleaned_wtennis["year"].min()} → {cleaned_wtennis["year"].max()}')
            print(f'  Championships: {(cleaned_wtennis["national_championship"] == "yes").sum()}')
    else:
        print('⚠ No women\'s tennis data found')
else:
...
if soccer_dfs:
    ...
    if not soccer_raw.empty:
        ...
        if not cleaned_soccer.empty:
            ...
            print(f'✓ Soccer: {len(cleaned_soccer)} seasons | {cleaned_soccer["year"].min()} → {cleaned_soccer["year"].max()}')
            print(f'  Championships: {(cleaned_soccer["national_championship"] == "yes").sum()}')
        else:
            print('⚠ No soccer data after cleaning')
    else:
...
if volleyball_dfs:
    ...
    if not volleyball_raw.empty:
        ...
        if not cleaned_volleyball.empty:
            ...
            print(f'✓ Volleyball: {len(cleaned_volleyball)} seasons | {cleaned_volleyball["year"].min()} → {cleaned_volleyball["year"].max()}')
            print(f'  Championships: {(cleaned_volleyball["national_championship"] == "yes").sum()}')
        else:
            print('⚠ No volleyball data after cleaning')
    else:
        print('⚠ No volleyball data found')
else:
    print('⚠ Could not scrape volleyball data')
